...
#   §H  doubleml_translation_row()  — cents/dollars translation-invariance row
...
# ==============================================================================
# §H  TRANSLATION-INVARIANCE ROW (cents vs dollars)
# ==============================================================================
# For one weight vector, builds a comparison row:
#   Dollars     = sum(w * Y_dollars)
#   Cents       = sum(w * Y_cents)
#   Diff_actual = Cents - Dollars
#   Diff_pred   = sum(w) * k          [theory: k * sum_i omega_i]
#   Match       = whether actual == predicted (machine precision)
# where k = Y_cents - Y_dollars = log(100).
#
# *** CHANGED vs notebook ***  Y_dollars, Y_cents, k are explicit arguments
# (were globals Y_dol/Y_cnt/k). Was also defined twice in the extended
# notebook; here it is defined once.

doubleml_translation_row <- function(name, w, Y_dollars, Y_cents, k) {
  ed <- sum(w * Y_dollars)
  ec <- sum(w * Y_cents)
  data.frame(
    Estimator   = name,
