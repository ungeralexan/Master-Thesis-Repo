...
Wald-AIPW & (DML) & n/a & \checkmark\,(std.) & not guaranteed & not guaranteed & Scale-norm. (std.)\tnote{a} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
  \small
  \item \textit{SUW norm.} refers to normalization in the sense of
  \citet{sloczynski2025abadie}: the kappa/IPW weights entering the estimator are
  separately rescaled before the Wald ratio is formed. \textit{Knaus class} refers
  to the normalization of the final outcome weights $\omega_i$ following
  \citet[Tables~4--5]{knaus2024treatment}. \checkmark\,(exact) means the condition
  holds algebraically in every finite sample.
  \item[$^{a}$] In standard flexible implementations, Wald-AIPW is scale-normalized
  but not generally fully normalized. Full normalization requires the additional
  smoother condition that also reproduces the treatment nuisance component
  \citep[Section~4.2.3]{knaus2024treatment}.
  \item[$^{b}$] For $\hat\tau_{a,1}$, the treated side is normalized because its
  denominator is the treated-side kappa total $\sum_i\kappa_{i1}$; the untreated side
  is not normalized in general. See Appendix~\ref{app:omega_a1}.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
% ==============================================================================
% APPENDIX — Outcome Weights of \hat\tau_{a,1}
%
% Paste into your appendix section, e.g. after \section{Proofs} or as its own
% \subsection{Outcome Weights of $\hat\tau_{a,1}$}
% ==============================================================================
\appendix
